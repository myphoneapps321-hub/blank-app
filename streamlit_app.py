...
def compute(centres, flux):
    ...
    for s in SECTEURS:
        sub = df[df["Secteur"] == s]
        if len(sub):
            rows.append({"Périmètre": f"Secteur {s}", **agg(sub)})
    for c in (1, 2):
...
def build_html_report(synthese, detail, periode):
    ...
    secteurs = synthese[synthese["Périmètre"].str.startswith("Secteur")]
    ...
    if worst_sec is not None:
        obs.append(f"Le secteur au rendement le plus faible est <b>{worst_sec['Périmètre']}</b> "
...
profil = st.sidebar.radio(
    "Profil / accès",
    ["Responsable DR"] + [f"Agent secteur {s}" for s in SECTEURS],
    help="Un agent de secteur ne saisit que son périmètre ; le responsable DR "
...
mon_secteur = None if is_dr else int(profil.split()[-1])
...
if is_dr:
    ...
    st.title(f"Saisie — Agent secteur {mon_secteur}")
    st.caption("Vous ne modifiez que les centres et les flux de votre secteur.")

col_cfg = {
    "Secteur": st.column_config.SelectboxColumn(options=SECTEURS, required=True),
...
with tabs[0]:
    if is_dr:
        ...
        st.subheader(f"Centres du secteur {mon_secteur}")
...
with tabs[2]:
    ...
    if not is_dr:
        ...
        synthese = synthese[synthese["Périmètre"] == f"Secteur {mon_secteur}"]
